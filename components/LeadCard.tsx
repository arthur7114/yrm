...
export default function LeadCard({ lead }: LeadCardProps) {
    ...
    const statusColors: Record<string, string> = {
        novo: 'bg-gray-100 text-gray-800 dark:bg-gray-700 dark:text-gray-200',
        em_qualificacao: 'bg-blue-100 text-blue-800 dark:bg-blue-900/50 dark:text-blue-300',
        aguardando_humano: 'bg-amber-100 text-amber-800 dark:bg-amber-900/50 dark:text-amber-300',
        em_atendimento_humano: 'bg-green-100 text-green-800 dark:bg-green-900/50 dark:text-green-300',
        encerrado: 'bg-emerald-100 text-emerald-800 dark:bg-emerald-900/50 dark:text-emerald-300',
    }
                    ...
                    <div className="mt-2 sm:flex sm:justify-between">
                        <div className="sm:flex">
                            <p className="flex items-center text-sm text-gray-500 dark:text-gray-400">
                                Status:
                                <span className={`ml-2 inline-flex items-center rounded px-2 py-0.5 text-xs font-medium ${statusColors[normalizedStatus || ''] || 'bg-gray-100 text-gray-800 dark:bg-gray-700 dark:text-gray-300'}`}>
                                    {normalizedStatus ? leadStatusLabels[normalizedStatus] : 'Sem status'}
                                </span>
                            </p>
                        </div>
